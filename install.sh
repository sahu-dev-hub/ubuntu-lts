...
install_ubuntu() {
    if [[ "$PD_VER" -ge 5 ]]; then
        # v5: Docker image style, --name se custom naam
        proot-distro install ubuntu:24.04 --name "$ds_name"
    else
        # v4: purana plugin-based method
        if [[ ! -f "$PREFIX/etc/proot-distro/$ds_name.sh" ]]; then
            download_script \
                ...
                "$PREFIX/etc/proot-distro/" silence
        fi
...
remove_ubuntu() {
    ...
    # Agar manually directory reh gayi ho toh bhi saaf karo
